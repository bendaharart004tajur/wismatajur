...
import { addPendapatanAction } from '@/app/actions/pendapatan-actions';
...
export function AddPendapatanDialog({ onSuccess }: { onSuccess: () => void }) {
  const { user } = useAuth();
  ...
  async function onSubmit(values: PendapatanFormValues) {
    ...
    try {
      ...
      const result = await addPendapatanAction(user.peran, dataToSend);
      ...
      toast({ title: 'Error', description: 'Terjadi kesalahan saat menambahkan data.', variant: 'destructive' });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
         <Button>
            <PlusCircle className="mr-2 h-4 w-4" /> Catat Pendapatan
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Catat Pendapatan Baru</DialogTitle>
          <DialogDescription>
            Isi detail pendapatan lain-lain di bawah ini.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <div className="grid gap-4 py-4">
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <Textarea placeholder="Contoh: Dana hibah dari kelurahan" {...field} />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
