...
interface InventarisClientPageProps {
  initialData: Inventaris[];
}

export function InventarisClientPage({ initialData }: InventarisClientPageProps) {
  const { user } = useAuth();
  const [data, setData] = useState<Inventaris[]>(initialData);
  ...
  const fetchInventaris = useCallback(() => {
    getInventarisAction()
      ...
      .catch(() => {
        toast({
          title: "Error",
          description: "Gagal memuat ulang data inventaris.",
          variant: "destructive",
        });
      });
  }, [toast]);
