...
interface PengumumanClientPageProps {
  initialData: Pengumuman[];
}

export function PengumumanClientPage({ initialData }: PengumumanClientPageProps) {
  const { user } = useAuth();
  const [data, setData] = useState<Pengumuman[]>(initialData);
  const [isLoading, setIsLoading] = useState(false); // To show loading on refresh
  ...
  const fetchPengumuman = useCallback(() => {
    ...
    getPengumumanAction()
      ...
      .catch(() => {
        toast({
          title: "Error",
          description: "Gagal memuat ulang data pengumuman.",
          variant: "destructive",
        });
      })
